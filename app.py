...
if uploaded_file is not None:
    ...
    for page_number in range(len(pdf_document)):
        ...
        pix = page.get_pixmap()
        image_path = f"output_image_{page_number + 1}.png"
        pix.save(image_path)
    ...
    if img_with_bbox_all_pages is not None:
        st.image(cv2.cvtColor(img_with_bbox_all_pages, cv2.COLOR_BGR2RGB), caption='EMBADDED NETWORK', use_column_width=True)
